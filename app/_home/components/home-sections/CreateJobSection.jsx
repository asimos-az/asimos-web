...
import { ArrowBackRounded, ArrowForwardRounded, BusinessCenterRounded, CheckCircleOutlineRounded, EmailOutlined, LocationOnOutlined, PhoneOutlined, SaveOutlined, ScheduleRounded, VisibilityOutlined } from "@mui/icons-material";
...
export default function CreateJobSection({ ctx }) {
  ...
  const qualityChecks = useMemo(() => [
    [Boolean(title.trim()), "Vəzifənin adı aydındır"], [Boolean(category), "Kateqoriya seçilib"],
    [description.trim().length >= 80, "Təsvir kifayət qədər ətraflıdır"],
    [Boolean(contactVisibility.phone || contactVisibility.whatsapp || contactVisibility.email), "Əlaqə üsulu seçilib"],
    [Boolean(locationText && lat && lng), "İş ünvanı müəyyən edilib"],
  ], [title, category, description, contactVisibility, locationText, lat, lng]);
  ...
  return <main className={pageStyles.page}>
    <header className={pageStyles.workspaceHeader}><div className={pageStyles.brand}><BusinessCenterRounded /><strong>{editingJobId ? "Vakansiyanı redaktə et" : "Yeni vakansiya yarat"}</strong></div><div className={pageStyles.topActions}><button type="button" onClick={(event) => handleCreateJob(event, true)} disabled={loading || Boolean(editingJobId)}><SaveOutlined /> Qaralama saxla</button><button type="button" onClick={() => setStep(4)}><VisibilityOutlined /> Önizlə</button><button type="button" aria-label="Bağla" onClick={() => setActiveSection("profile")}>×</button></div></header>

    <nav className={pageStyles.stepper} aria-label="Vakansiya yaratma addımları">{steps.map((label, index) => <button type="button" key={label} className={index === step ? pageStyles.currentStep : index < step ? pageStyles.doneStep : ""} onClick={() => setStep(index)}><span>{index < step ? "✓" : index + 1}</span><small>{label}</small></button>)}</nav>

    <div className={pageStyles.layout}><form className={pageStyles.formCard} onSubmit={handleCreateJob}>
      ...
      {step === 4 ? <div className={pageStyles.previewStep}><div className={pageStyles.previewJob}><div className={pageStyles.previewCompany}><BusinessCenterRounded /><span>{user?.companyName || user?.company_name || companyObject || "Şirkət"}</span></div><h2>{title || "Vakansiyanın adı"}</h2><div className={pageStyles.previewTags}><span>{category || "Kateqoriya"}</span><span>{jobLevel || "Dərəcə"}</span><span>{jobType === "temporary" ? "Müvəqqəti" : "Daimi"}</span></div><p>{description || "Vakansiya təsviri burada görünəcək."}</p><div className={pageStyles.previewMeta}><span><LocationOnOutlined />{locationText || "Ünvan seçilməyib"}</span><span><ScheduleRounded />{scheduleStart && scheduleEnd ? `${scheduleStart}–${scheduleEnd}` : "Qrafik qeyd edilməyib"}</span></div></div><div className={pageStyles.publishBox}><label><span>Elanı indi göndər</span><input type="radio" checked={publishMode === "instant"} onChange={() => setPublishMode("instant")} /></label><label><span>Yayımı planlaşdır</span><input type="radio" checked={publishMode === "scheduled"} onChange={() => setPublishMode("scheduled")} /></label>{publishMode === "scheduled" ? <input type="datetime-local" value={publishAt} onChange={(event) => setPublishAt(event.target.value)} /> : null}<p>Elan admin təsdiqindən sonra yayımlanacaq.</p></div></div> : null}

      <footer className={pageStyles.formFooter}><button type="button" className={pageStyles.cancel} onClick={step ? previous : () => setActiveSection("profile")}><ArrowBackRounded />{step ? "Geri" : "Ləğv et"}</button><div>{!editingJobId ? <button type="button" className={pageStyles.draft} onClick={(event) => handleCreateJob(event, true)} disabled={loading}><SaveOutlined /> Qaralama saxla</button> : <button type="button" className={pageStyles.draft} onClick={resetJobForm}>Redaktəni ləğv et</button>}{step < 4 ? <button type="button" className={pageStyles.next} onClick={next}>Növbəti: {steps[step + 1]} <ArrowForwardRounded /></button> : <button type="submit" className={pageStyles.next} disabled={loading}>{loading ? "Göndərilir..." : editingJobId ? "Dəyişiklikləri saxla" : "Elanı adminə göndər"}<ArrowForwardRounded /></button>}</div></footer>
    </form>

    <aside className={pageStyles.sidebar}><section><h2>Elan keyfiyyəti</h2><div className={pageStyles.quality}><div style={{ "--progress": `${quality * 3.6}deg` }}>{quality}%</div><p><strong>{quality >= 80 ? "Əla görünür!" : "Yaxşı başlanğıcdır!"}</strong><span>Daha çox namizədə çatmaq üçün sahələri tamamlayın.</span></p></div><ul>{qualityChecks.map(([done,label]) => <li className={done ? pageStyles.checked : ""} key={label}><CheckCircleOutlineRounded />{label}</li>)}</ul></section><section><h2>Canlı önizləmə</h2><div className={pageStyles.miniPreview}><BusinessCenterRounded /><strong>{user?.companyName || user?.company_name || "Şirkət"}</strong><h3>{title || "Vakansiyanın adı"}</h3><span>{category || "Kateqoriya seçilməyib"}</span><p>{description.slice(0, 130) || "Qısa vakansiya məlumatı burada görünəcək."}</p><small>{locationText || "Ünvan seçilməyib"}</small></div></section></aside></div>
  </main>;
}
