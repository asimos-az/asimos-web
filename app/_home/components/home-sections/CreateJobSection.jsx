...
export default function CreateJobSection({ ctx }) {
  ...
  return <main className={pageStyles.page}>
    ...
    <aside className={pageStyles.sidebar}><section><h2>Elan keyfiyyəti</h2><div className={pageStyles.quality}><div style={{ "--progress": `${quality * 3.6}deg` }}>{quality}%</div><p><strong>{quality >= 80 ? "Əla görünür!" : quality >= 40 ? "Yaxşı başlanğıcdır!" : "Məlumatları tamamlayın"}</strong><span>Faiz yalnız daxil etdiyiniz və yoxladığınız məlumatlara əsaslanır.</span></p></div><ul>{qualityChecks.map(([done,label]) => <li className={done ? pageStyles.checked : ""} key={label}><CheckCircleOutlineRounded />{label}</li>)}</ul></section><section><h2>Canlı önizləmə</h2><div className={pageStyles.miniPreview}><div className={pageStyles.miniCompany}><BusinessCenterRounded /><div><strong>{user?.companyName || user?.company_name || companyObject || "Şirkət"}</strong><small><VerifiedRounded /> Təsdiqlənmiş şirkət</small></div></div><h3>{title || "Vakansiyanın adı"}</h3><strong className={pageStyles.miniSalary}>{wageLabel}</strong><div className={pageStyles.miniTags}><span>{category || "Kateqoriya"}</span><span>{workTypeLabel}</span></div><p>{description.slice(0, 150) || "Qısa vakansiya məlumatı burada görünəcək."}</p><div className={pageStyles.miniMeta}><span><LocationOnOutlined />{locationText || "Ünvan seçilməyib"}</span><span><PaymentsOutlined />{wageLabel}</span></div><button type="button" tabIndex={-1}>1 kliklə müraciət et</button></div></section></aside></div>
  </main>;
}
