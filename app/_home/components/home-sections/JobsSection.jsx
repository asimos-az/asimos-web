  ...
  return <main className={pageStyles.page}>
    ...
    <section className={pageStyles.filterCard} aria-label="Vakansiya filterləri">
      <form className={pageStyles.primaryFilters} onSubmit={runSearch}>
        <label className={pageStyles.searchField}><span aria-hidden="true">⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Vəzifə, şirkət və ya açar söz" aria-label="Vakansiya axtar" /></label>
        <label className={pageStyles.selectField}><span aria-hidden="true">⌖</span><select value={city} onChange={(event) => setCity(event.target.value)} aria-label="Şəhər, rayon və ya metro"><option value="">Şəhər, rayon və ya metro</option>{cityOptions.map((item) => <option key={item}>{item}</option>)}</select></label>
        <button className={pageStyles.searchButton} disabled={loading} type="submit">{loading ? "Axtarılır..." : "Axtar"}</button>
