...
export default function JobsSection({ ctx }) {
  ...
  useEffect(() => {
    ...
    if (!mapJobId) return;
    setFocusedMapJobId(mapJobId);
    window.requestAnimationFrame(() => document.querySelector("[class*='mapPanel']")?.scrollIntoView({ behavior: "smooth", block: "center" }));
  }, [activeSection, setFocusedMapJobId]);
  ...
  return <main className={pageStyles.page}>
    ...
    <section className={pageStyles.explorer}>
      ...
      <aside className={pageStyles.mapPanel}><JobsMap jobs={shownJobs} seekers={user ? seekersOnMap : []} showSeekers={Boolean(user)} focusedJobId={focusedMapJobId} userLocation={effectiveLocation} radiusM={Number(homeRadiusM)} /></aside>
    </section>
  </main>;
}
