    ...
    <Container maxWidth="xl" sx={{ mt: { xs: 4, md: 5 } }}>
      ...
      <Dialog open={Boolean(mapJob)} onClose={() => setMapJobId(null)} fullWidth maxWidth="xl" PaperProps={{ className: styles.jobsMapDialog }} aria-labelledby="nearby-jobs-map-title">
        ...
        <DialogContent className={styles.jobsMapDialogContent}>
          <Box className={styles.jobsMapDialogMap}><ctx.JobsMap jobs={modalMapJobs} seekers={ctx.user?.role === "employer" ? ctx.seekersOnMap : []} showSeekers={ctx.user?.role === "employer"} focusedJobId={mapJob?.id || null} userLocation={ctx.effectiveLocation} radiusM={Number(ctx.homeRadiusM) || 30000} /></Box>
