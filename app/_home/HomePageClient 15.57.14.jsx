  ...
  return (
    <main className="site-shell">
      ...
      {activeSection === "create" && canCreateJob ? (
        <section className="container page-section" style={{ maxWidth: 980 }}>
          ...
          {user ? (
            <form onSubmit={handleCreateJob} style={{ display: "grid", gap: 18 }}>
              <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 28, padding: "26px 28px", boxShadow: "0 18px 45px rgba(15,23,42,.06)" }}>
                <div style={{ color: "#9a9a9a", fontSize: 16, fontWeight: 800, letterSpacing: 1.6, textTransform: "uppercase", marginBottom: 24 }}>Vakansiya məlumatları</div>
                <div style={{ display: "grid", gap: 18 }}>
                  <label style={{ display: "grid", gap: 8, fontSize: 18, color: "#5f5f64" }}>
                    Vakansiya adı <span style={{ color: "#ef4444" }}>*</span>
                    <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Məs: Mühasib, React proqramçısı..." required style={{ width: "100%", border: "1px solid #dbe3ee", borderRadius: 22, minHeight: 64, padding: "0 24px", fontSize: 22, outline: "none" }} />
                  </label>

                  <label style={{ display: "grid", gap: 8, fontSize: 18, color: "#5f5f64" }}>
                    Filial / iş yeri
                    <input value={companyObject} onChange={(e) => setCompanyObject(e.target.value)} placeholder="Məs: Nərimanov filialı, Mərkəzi ofis..." style={{ width: "100%", border: "1px solid #dbe3ee", borderRadius: 22, minHeight: 64, padding: "0 24px", fontSize: 22, outline: "none" }} />
                  </label>

                  <label style={{ display: "grid", gap: 8, fontSize: 18, color: "#5f5f64" }}>
                    Kateqoriya <span style={{ color: "#ef4444" }}>*</span>
                    <select value={category} onChange={(e) => setCategory(e.target.value)} required style={{ width: "100%", border: "1px solid #dbe3ee", borderRadius: 22, minHeight: 64, padding: "0 24px", fontSize: 22, background: "#fff", outline: "none" }}>
                  ...
                  <label style={{ display: "grid", gap: 8, fontSize: 18, color: "#5f5f64" }}>
                    Dərəcə
                    <select value={jobLevel} onChange={(e) => setJobLevel(e.target.value)} style={{ width: "100%", border: "1px solid #dbe3ee", borderRadius: 22, minHeight: 64, padding: "0 24px", fontSize: 22, background: "#fff", outline: "none" }}>
              ...
              <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 28, padding: "26px 28px", boxShadow: "0 18px 45px rgba(15,23,42,.06)" }}>
                <div style={{ color: "#9a9a9a", fontSize: 16, fontWeight: 800, letterSpacing: 1.6, textTransform: "uppercase", marginBottom: 24 }}>Maaş <span style={{ color: "#ef4444" }}>*</span></div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginBottom: wageMode === "range" ? 18 : 8 }}>
                  {[
                    ["agreement", "Razılaşma"],
                    ["skill", "Bacarığa əsasən"],
                    ["range", "Rəqəm göstər"],
                  ].map(([value, label]) => (
                    <button key={value} type="button" onClick={() => { setWageMode(value); if (value === "agreement") setWage("Razılaşma əsasında"); if (value === "skill") setWage("Bacarığa uyğun"); }} style={{ border: "1px solid #dbe3ee", borderRadius: 999, minHeight: 58, padding: "0 28px", fontSize: 20, fontWeight: 700, background: wageMode === value ? "#1fa276" : "#fff", color: wageMode === value ? "#fff" : "#666", cursor: "pointer" }}>
                      {label}
                    </button>
                  ))}
                </div>
                {wageMode === "range" ? (
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 12 }}>
                    <input value={wageMin} onChange={(event) => { const value = event.target.value.replace(/[^0-9]/g, ""); setWageMin(value); const max = String(wageMax || "").replace(/[^0-9]/g, ""); setWage(value && max ? `${value} - ${max} AZN` : value ? `${value} AZN` : ""); }} inputMode="numeric" placeholder="Minimum maaş" style={{ border: "1px solid #dbe3ee", borderRadius: 18, minHeight: 56, padding: "0 18px", fontSize: 18 }} />
                    <input value={wageMax} onChange={(event) => { const value = event.target.value.replace(/[^0-9]/g, ""); setWageMax(value); const min = String(wageMin || "").replace(/[^0-9]/g, ""); setWage(min && value ? `${min} - ${value} AZN` : min ? `${min} AZN` : value ? `${value} AZN` : ""); }} inputMode="numeric" placeholder="Maksimum maaş" style={{ border: "1px solid #dbe3ee", borderRadius: 18, minHeight: 56, padding: "0 18px", fontSize: 18 }} />
                  </div>
                ) : <p style={{ margin: "10px 0 0", color: "#9a9a9a", fontSize: 17 }}>Elanda "{wageMode === "skill" ? "Bacarığa uyğun" : "Razılaşma əsasında"}" görünəcək</p>}
              </div>

              <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 28, padding: "26px 28px", boxShadow: "0 18px 45px rgba(15,23,42,.06)" }}>
                <div style={{ color: "#9a9a9a", fontSize: 16, fontWeight: 800, letterSpacing: 1.6, textTransform: "uppercase", marginBottom: 24 }}>İş növü <span style={{ color: "#ef4444" }}>*</span></div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginBottom: 22 }}>
                  <button type="button" onClick={() => { setJobType("permanent"); setDurationPreset("1"); }} style={{ border: "1px solid #dbe3ee", borderRadius: 999, minHeight: 58, padding: "0 28px", fontSize: 20, fontWeight: 700, background: jobType !== "temporary" ? "#1fa276" : "#fff", color: jobType !== "temporary" ? "#fff" : "#666", cursor: "pointer" }}>Daimi iş</button>
                  <button type="button" onClick={() => { setJobType("temporary"); setDurationPreset("1"); setDurationDays("1"); }} style={{ border: "1px solid #dbe3ee", borderRadius: 999, minHeight: 58, padding: "0 28px", fontSize: 20, fontWeight: 700, background: jobType === "temporary" ? "#1fa276" : "#fff", color: jobType === "temporary" ? "#fff" : "#666", cursor: "pointer" }}>Günəmuzd</button>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 16 }}>
                  <label style={{ display: "grid", gap: 8, fontSize: 18, color: "#5f5f64" }}>Başlama tarixi
                    <input type="date" value={vacancyStartDate} onChange={(e) => setVacancyStartDate(e.target.value)} style={{ border: "1px solid #dbe3ee", borderRadius: 18, minHeight: 58, padding: "0 18px", fontSize: 20 }} />
                  </label>
                  <label style={{ display: "grid", gap: 8, fontSize: 18, color: "#5f5f64" }}>Bitmə tarixi <span style={{ color: "#ef4444" }}>*</span>
                    <input type="date" value={vacancyEndDate} onChange={(e) => setVacancyEndDate(e.target.value)} required style={{ border: "1px solid #dbe3ee", borderRadius: 18, minHeight: 58, padding: "0 18px", fontSize: 20 }} />
                  </label>
                </div>
              </div>

              <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 28, padding: "26px 28px", boxShadow: "0 18px 45px rgba(15,23,42,.06)" }}>
                <div style={{ color: "#9a9a9a", fontSize: 16, fontWeight: 800, letterSpacing: 1.6, textTransform: "uppercase", marginBottom: 24 }}>Əlaqə məlumatları</div>
                <div style={{ background: "#eef4ff", borderRadius: 18, padding: 18, display: "grid", gap: 14 }}>
                  ...
                  {[
                    ...
                    <div key={key} style={{ display: "grid", gridTemplateColumns: "76px 32px minmax(0, 1fr) auto", gap: 12, alignItems: "center" }}>
                      <button type="button" onClick={() => setContactVisibility((prev) => ({ ...prev, [key]: !prev[key] }))} aria-pressed={Boolean(contactVisibility[key])} style={{ width: 64, height: 36, borderRadius: 999, border: 0, padding: 4, background: contactVisibility[key] ? "#1fa276" : "#cfd3d8", cursor: "pointer", display: "flex", justifyContent: contactVisibility[key] ? "flex-end" : "flex-start" }}><span style={{ width: 28, height: 28, borderRadius: "50%", background: "#fff", display: "block" }} /></button>
                      <span style={{ fontSize: 22 }}>{icon}</span>
                      <span style={{ fontSize: 20, color: "#111827", overflow: "hidden", textOverflow: "ellipsis" }}>{value}</span>
                      {primaryContact === key ? <span style={{ background: "#dcfce7", color: "#147555", borderRadius: 999, padding: "8px 16px", fontWeight: 800 }}>İlk göstərilməlidir</span> : <button type="button" onClick={() => setPrimaryContact(key)} style={{ border: "1px solid #6ee7b7", color: "#15956d", background: "#fff", borderRadius: 14, padding: "9px 16px", fontWeight: 800, cursor: "pointer" }}>İlk et</button>}
                    </div>
                  ))}
                  <p style={{ margin: 0, color: "#9a9a9a", fontSize: 17 }}>Aktiv etdiyiniz əlaqə yolları elanda göstərilir. “İlk göstərilməlidir” seçimi həmin əlaqə vasitəsini siyahının başına çıxarır.</p>
                </div>
                <label style={{ display: "grid", gap: 8, fontSize: 18, color: "#5f5f64", marginTop: 18 }}>Daxili CV bazası
                  <input value={link} onChange={(e) => setLink(e.target.value)} placeholder="https://ats.sirketiniz.az/apply" style={{ width: "100%", border: "1px solid #dbe3ee", borderRadius: 18, minHeight: 58, padding: "0 20px", fontSize: 20 }} />
                </label>
              </div>

              <label style={{ display: "grid", gap: 8, fontSize: 18, color: "#5f5f64", background: "#fff", border: "1px solid #e5e7eb", borderRadius: 28, padding: "26px 28px", boxShadow: "0 18px 45px rgba(15,23,42,.06)" }}>
                Təsvir
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={5} required placeholder="Vakansiyanın tələbləri, vəzifələr və əlavə qeydlər..." style={{ width: "100%", border: "1px solid #dbe3ee", borderRadius: 18, padding: 18, fontSize: 18, resize: "vertical" }} />
              </label>

              <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 28, padding: "26px 28px", boxShadow: "0 18px 45px rgba(15,23,42,.06)" }}>
                <div style={{ color: "#9a9a9a", fontSize: 16, fontWeight: 800, letterSpacing: 1.6, textTransform: "uppercase", marginBottom: 22 }}>Lokasiya <span style={{ color: "#ef4444" }}>*</span></div>
                <div style={{ border: "1px solid #a7f3d0", borderRadius: 20, overflow: "hidden", minHeight: 230 }}>
              ...
              <div style={{ background: "#f8fbff", border: "1px solid #bfdbfe", borderRadius: 26, padding: "22px 26px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 18 }}>
                <strong style={{ color: "#1d5fae", fontSize: 22 }}>📅 Yayımlanma planlaması</strong>
                <button type="button" onClick={() => setPublishMode((value) => value === "scheduled" ? "instant" : "scheduled")} aria-pressed={publishMode === "scheduled"} style={{ width: 72, height: 42, borderRadius: 999, border: 0, padding: 5, background: publishMode === "scheduled" ? "#1fa276" : "#cfd3d8", cursor: "pointer", display: "flex", justifyContent: publishMode === "scheduled" ? "flex-end" : "flex-start" }}><span style={{ width: 32, height: 32, borderRadius: "50%", background: "#fff", display: "block" }} /></button>
              </div>

              {publishMode === "scheduled" ? (
                <label style={{ display: "grid", gap: 8, fontSize: 18, color: "#5f5f64" }}>Yayım tarixi və saatı
                  <input type="datetime-local" value={publishAt} onChange={(e) => setPublishAt(e.target.value)} style={{ border: "1px solid #dbe3ee", borderRadius: 18, minHeight: 58, padding: "0 18px", fontSize: 20 }} />
                </label>
              ) : null}

              <p style={{ color: "#747b87", fontSize: 18, lineHeight: 1.55, margin: "8px 0" }}>🛡️ Elan adminə göndəriləcək. Təsdiqləndikdən sonra yayımlanacaq. Daha sürətli təsdiq almaq üçün elanı qaydalara uyğun və ətraflı formada doldurun.</p>

              <div style={{ display: "grid", gap: 12 }}>
                {!editingJobId ? <button type="button" disabled={loading} onClick={(e) => handleCreateJob(e, true)} style={{ minHeight: 64, border: "1px solid #dbe3ee", borderRadius: 22, background: "#fff", color: "#555", fontSize: 22, fontWeight: 800, cursor: "pointer" }}>✏️ Qaralama olaraq saxla</button> : null}
                <button type="submit" disabled={loading} style={{ minHeight: 66, border: 0, borderRadius: 22, background: "#1fa276", color: "#fff", fontSize: 22, fontWeight: 900, cursor: loading ? "not-allowed" : "pointer", opacity: loading ? .7 : 1 }}>{loading ? "Göndərilir..." : editingJobId ? "Dəyişiklikləri saxla" : "📥 Elanı adminə göndər"}</button>
                {editingJobId ? <button type="button" onClick={resetJobForm} style={{ minHeight: 54, border: "1px solid #dbe3ee", borderRadius: 18, background: "#fff", color: "#555", fontSize: 18, fontWeight: 800, cursor: "pointer" }}>Redaktəni ləğv et</button> : null}
              </div>
            </form>
          ) : null}
        </section>
