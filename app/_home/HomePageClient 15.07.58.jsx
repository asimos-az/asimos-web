...
function FloatingHomeWidgets({ config }) {
  ...
  const ideaEmail = getWidgetValue(idea, "emailTo", "email_to", "");
  const ideaSubject = getWidgetValue(idea, "emailSubject", "email_subject", "Asimos.az üçün yeni ideya");
  ...
  const mailHref = `mailto:${encodeURIComponent(ideaEmail)}?subject=${encodeURIComponent(ideaSubject)}&body=${encodeURIComponent(ideaText || "")}`;
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
              <h2 style={{ margin: 0, color: "#0f172a", fontSize: 28, fontWeight: 900 }}>💡 {ideaTitle}</h2>
              <button
                type="button"
                onClick={() => setIdeaOpen(false)}
                aria-label="Bağla"
                style={{ border: 0, background: "transparent", fontSize: 42, color: "#8b8f98", cursor: "pointer", lineHeight: 1 }}
              >
                ×
              </button>
            </div>

            <p style={{ color: "#6b7280", fontSize: 22, lineHeight: 1.5, margin: "26px 0" }}>
              {ideaDescription}
            </p>
            ...
            <a
              href={ideaEmail ? mailHref : undefined}
              onClick={(event) => { if (!ideaEmail) event.preventDefault(); }}
              style={{
                marginTop: 28,
                minHeight: 70,
                borderRadius: 20,
