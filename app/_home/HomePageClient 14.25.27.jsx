...
export default function HomePageClient() {
  ...
  useEffect(() => {
    ...
    fetch(`${SOCKET_URL}/sponsored-card`)
      .then((res) => (res.ok ? res.json() : { item: null }))
      .then((data) => {
        if (!ignore) setSponsoredCard(data?.item || null);
      })
      .catch(() => {
        if (!ignore) setSponsoredCard(null);
      });
    ...
    <main className="site-shell">
          ...
          {hasHomeJobs || sponsoredCard ? (
            <section className="container page-section">
              ...
              <div className={styles.latestJobsGridList}>
                ...
                {homeJobs.slice(0, sponsoredCard ? 7 : 8).map((job) => (
      ...
      {activeSection === "jobs" ? (
        <section className="container page-section">
          ...
          {shownJobs.length === 0 && !sponsoredCard ? (
