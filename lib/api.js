...
async function request(path, { method = "GET", body, retry = 0 } = {}) {
  ...
  if (typeof window !== "undefined") headers["Accept-Language"] = window.localStorage.getItem("asimos_locale") || "az";
...
export const api = {
  ...
  listJobsWithSearch: ({ q, lat, lng, radius_m, daily, jobType, jobLevel, city, minWage, maxWage, categories, page, limit }) =>
    request(`/jobs${qs({ q, lat, lng, radius_m, daily, jobType, jobLevel, city, minWage, maxWage, categories: Array.isArray(categories) ? categories.join(",") : categories, page, limit })}`),
  listJobs: () => request("/jobs?limit=1000"),
  listCompanies: ({ q, page = 1, limit = 100 } = {}) => request(`/companies${qs({ q, page, limit })}`),
  getCompany: (id) => request(`/companies/${encodeURIComponent(String(id))}`),
  getJobFilterOptions: () => request("/job-filter-options"),
  getCityDirectory: () => request("/cities"),
  ...
  listMyFavorites: () => request("/me/favorites"),
  addFavoriteJob: (id) => request(`/jobs/${encodeURIComponent(String(id))}/favorite`, { method: "POST" }),
  removeFavoriteJob: (id) => request(`/jobs/${encodeURIComponent(String(id))}/favorite`, { method: "DELETE" }),
  registerJobView: (id) => request(`/jobs/${encodeURIComponent(String(id))}/view`, { method: "POST" }),
  ...
  publishJob: (id) => request(`/jobs/${encodeURIComponent(String(id))}/publish`, { method: "PATCH" }),
  deleteJob: (id) => request(`/jobs/${encodeURIComponent(String(id))}`, { method: "DELETE" }),

  listCategories: () => request("/categories"),
  getContent: (slug) => request(`/content/${encodeURIComponent(slug)}`),
  getSiteStats: () => typeof window === "undefined" ? request("/site-stats") : fetch("/api/site-stats", { cache: "no-store", headers: { Accept: "application/json" } }).then(async (res) => { const data = await res.json().catch(() => ({})); if (!res.ok) throw new Error(data?.error || "Statistikaları yükləmək mümkün olmadı"); return data; }),
  ...
  listSeekersForEmployerMap: () => request("/employers/seekers/map"),
